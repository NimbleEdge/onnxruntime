...
Status PadOpBuilder::AddToModelBuilderImpl(ModelBuilder& model_builder, const NodeUnit& node_unit) const {
  ...
  Initializer pads_initializer_raw_data(model_builder.GetGraphViwer().GetGraph(), *pads_initializer);
  ...
  if (inputs.size() > 2 && inputs[2].node_arg.Exists()) {
    ...
    Initializer pad_value_raw_data_init(model_builder.GetGraphViwer().GetGraph(), *constant_value_initializer);
